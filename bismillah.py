...
# 2️⃣  DATA PEGAWAI  (CRUD)
# ------------------------------------------------
elif page == "Data Pegawai":
    df = get_df()
    st.subheader("\U0001F465 Manajemen Data Pegawai")
        ...
        if df.empty:
            ...
            if not ori.empty:
                ...
                with st.form("form_edit_data"):
                    ...
                    with c2:
                        ttl_str = r["TL"]
                        try: ttl_obj = datetime.strptime(ttl_str, "%d/%m/%Y")
                        except: ttl_obj = datetime.today()
    ...
    # Tampilkan Data Tanpa Kolom "No"
    if not df.empty:
        st.markdown("### 🔍 Data Pegawai Saat Ini")
        st.dataframe(df.drop(columns=["No"], errors="ignore").reset_index(drop=True))
